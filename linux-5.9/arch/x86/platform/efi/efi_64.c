...
#ifdef CONFIG_NESTED_KERNEL
	__nk_entry(NK_DECLARE_PGD, (unsigned long)efi_pgd,
		   PGD_ALLOCATION_ORDER, NK_PTP_PGD);
#endif
...
#ifdef CONFIG_NESTED_KERNEL
	__nk_entry(NK_EFI_PGTBL_MEMCPY, (unsigned long)pgd_efi,
		   (unsigned long)pgd_k, sizeof(pgd_t) * num_entries);
#else
...
#ifdef CONFIG_NESTED_KERNEL
	__nk_entry(NK_EFI_PGTBL_MEMCPY, (unsigned long)p4d_efi,
		   (unsigned long)p4d_k, sizeof(p4d_t) * num_entries);
#else
...
#ifdef CONFIG_NESTED_KERNEL
	__nk_entry(NK_EFI_PGTBL_MEMCPY, (unsigned long)pud_efi,
		   (unsigned long) pud_k, sizeof(pud_t) * num_entries);
#else
