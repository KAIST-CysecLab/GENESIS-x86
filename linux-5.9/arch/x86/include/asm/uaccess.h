...
#elif defined(CONFIG_GENESIS_LABEL_CFI)
#define get_user(x, ptr)						\
({									\
	int __ret_gu;							\
	register __inttype(*(ptr)) __val_gu asm("%"_ASM_DX);		\
	__chk_user_ptr(ptr);						\
	might_fault();							\
	asm volatile("call __nk_get_user_%P4\n\t"			\
		     "nopl 0x1337beef\n\t"				\
		     : "=a" (__ret_gu), "=r" (__val_gu),		\
			ASM_CALL_CONSTRAINT				\
		     : "0" (ptr), "i" (sizeof(*(ptr)))			\
...
#define __put_user_x(size, x, ptr, __ret_pu)				\
	asm volatile("call __nk_put_user_" #size "\n\t"			\
		     "nopl 0x1337beef\n\t"				\
		     : "=a" (__ret_pu),					\
		       ASM_CALL_CONSTRAINT				\
		     : "0" ((typeof(*(ptr)))(x)), "c" (ptr)		\
		     : "cc", "ebx", "memory", "rdi", "rsi",		\
		       "rdx", "r8", "r9", "r10", "r11")
#else /* Non-CFI NESTED KERNEL */
#define get_user(x, ptr)						\
({									\
	int __ret_gu;							\
	register __inttype(*(ptr)) __val_gu asm("%"_ASM_DX);		\
