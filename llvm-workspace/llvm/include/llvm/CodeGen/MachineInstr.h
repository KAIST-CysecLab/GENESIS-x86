...
public:
  ...
  enum CFIFlag {
    NoCFIFlags   = 0,
    ShdwStkRet   = 1 << 0,              // Instruction is used as a return.
                                        // Should skip this instruction during
                                        // the CFI instrumentation.
  };
...
private:
  ...
  uint16_t CFIFlags = 0;                // Genesis Flags
...
public:
  ...
  /// Return whether a CFI flag is set.
  bool getCFIFlag(CFIFlag Flag) const {
    return CFIFlags & Flag;
  }

  /// Set a CFI flag.
  void setCFIFlag(CFIFlag Flag) {
    CFIFlags |= (uint16_t)Flag;
  }
