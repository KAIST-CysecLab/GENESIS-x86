...
static cl::opt<bool> EnableGenesisCFI("enable-x86-genesis-cfi",
				      cl::desc("Enable genesis label-based cfi"),
				      cl::init(false), cl::Hidden);

cl::opt<bool> EnableGenesisShadowStack("enable-x86-genesis-shadow-stack",
				      cl::desc("Enable genesis shadow stack"),
				      cl::init(false), cl::Hidden);
...
void X86PassConfig::addPreEmitPass() {
  ...
  if (EnableGenesisShadowStack) {
    addPass(createX86GenesisShadowStack());
  }

  if (EnableGenesisCFI) {
    addPass(createX86GenesisLabelCFI());
  }
}
